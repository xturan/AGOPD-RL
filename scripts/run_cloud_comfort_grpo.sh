...
# 数据:train_comfort.parquet(2752 题,1.7B p̂ 0.125-0.75)
...
NAME=${EXPERIMENT_NAME:-cloud_17_comfort_grpo_smoke}
STEPS=${TRAIN_STEPS:-10}

echo "[$(date +%H:%M)] START $NAME steps=$STEPS"
env CUDA_VISIBLE_DEVICES=0,1,2,3 STUDENT_GPUS=${STUDENT_GPUS:-4} TEACHER_N_GPUS=1 TEACHER_TENSOR_MODEL_PARALLEL_SIZE=1 \
    ...
    TRAIN_STEPS=${STEPS} TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-12} PPO_MINI_BATCH_SIZE=${PPO_MINI_BATCH_SIZE:-6} ROLLOUT_N=${ROLLOUT_N:-4} SAVE_FREQ=10 \
    ...
    actor_rollout_ref.actor.optim.lr=${LR:-1e-4} actor_rollout_ref.actor.kl_loss_coef=${KL_COEF:-0.01} actor_rollout_ref.model.lora_rank=${LORA_RANK:-16} trainer.save_freq=10 \
    > /tmp/${NAME}.log 2>&1

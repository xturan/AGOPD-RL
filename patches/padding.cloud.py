...
def left_right_2_no_padding(data: TensorDict) -> TensorDict:
    ...
    try:
        ...
        if "flash_attn" not in str(exc):
            raise
        # Keep the NestedTensor contract without requiring flash-attn.
        valid = attention_mask.bool()
        indices = valid.reshape(-1).nonzero(as_tuple=False).squeeze(-1)
        input_ids_rmpad = input_ids.reshape(-1, 1).index_select(0, indices)
        lengths = valid.sum(dim=1, dtype=torch.int32)
        cu_seqlens = torch.cat(
            [torch.zeros(1, dtype=torch.int32, device=lengths.device), lengths.cumsum(dim=0)]
        )

    def compact_first_axis(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if routed_experts is not None and not routed_experts.is_nested:
        ...
        routed_experts_rmpad = compact_first_axis(routed_experts.unsqueeze(-1))
    ...
    if teacher_logprobs is not None and teacher_ids is not None:
        teacher_logprobs_rmpad = compact_first_axis(teacher_logprobs.unsqueeze(-1))
        teacher_ids_rmpad = compact_first_axis(teacher_ids.unsqueeze(-1))
